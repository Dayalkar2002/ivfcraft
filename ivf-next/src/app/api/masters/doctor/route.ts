...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const action = body.docId ? 'update' : 'insert';
    const data = await masterService.saveDoctor(body, action);
    return NextResponse.json({ success: true, message: 'Doctor saved.', data });
  } catch (error: unknown) {
    return NextResponse.json({ success: false, message: (error as Error).message }, { status: 500 });
  }
}
